...
func AddKubeletConfigFlags(mainfs *pflag.FlagSet, c *kubeletconfig.KubeletConfiguration) {
	...
	fs.Var(cliflag.NewBracketSeparatedSliceMapStringString(&c.PreReservedMemoryZone), "pre-reserved-memory-zone", "A braket separated list of configuration for mamory manager(e.g. {numa-node=0, memory-type=memory, limit=1Gi}, {numa-node=1, memory-type=memory, limit=1Gi})")
}
