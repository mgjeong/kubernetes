...
type Manager interface {
	...
	// GetAllocatedResources return the CPU affinity for the container
...
func (m *manager) GetAllocatedResources(pod *v1.Pod, container *v1.Container) string {
	affinity := m.state.GetCPUSetOrDefault(string(pod.UID), container.Name)

	klog.Infof("[cpumanager] (Pod: %s, Container: %s) CPU affinity %s", pod.Name, container.Name, affinity)
