...
var (
	...
	fakePolicyStatic, _               = NewPolicyStatic(&fakeMachineInfo, fakeReserved, topologymanager.NewFakeManager())
			...
			if testCase.expErr == nil {
				if mgr != nil {
					...
					if testCase.policyName == "single-numa" {
						if !reflect.DeepEqual(rawMgr.policy.(*staticPolicy).systemReserved, fakeReserved) {
							t.Errorf("Memory Manager NewManager() error, expected systemReserved %+v but got: %+v",
								fakeReserved, rawMgr.policy.(*staticPolicy).systemReserved)
						}
					}
				} else {
