...
func (m *BracketSeparatedSliceMapStringString) Set(value string) error {
	...
	// spilt here
	//{numa-node=0,memory-type=memory,limit=1Gi},{numa-node=1,memory-type=memory,limit=1Gi}
	for _, splited := range strings.Split(value, "{") {
		splited = strings.TrimRight(splited, ",")
		splited = strings.TrimRight(splited, "}")
		...
		if err := tmpMap.Set(splited); err != nil {
...
func (m *BracketSeparatedSliceMapStringString) String() string {
	...
	for _, configMap := range *m.Value {
		...
		for key, str := range configMap {
			tmpStrs = append(tmpStrs, fmt.Sprintf("%s=%s", key, str))
		}

		if len(tmpStrs) != 0 {
			slices = append(slices, "{" + strings.Join(tmpStrs, ",") + "}")
		}
	}
	sort.Strings(slices)
